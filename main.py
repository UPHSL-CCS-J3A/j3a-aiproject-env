...
class PostureDetector:
    def __init__(self, ref_values, settings, sounds, gif_holder):
        self.gif_holder = {
    ...
    def run(self):
        while True:
            ...
            if res.pose_landmarks:
                ...
                color = (0,255,0) if (ref_values["calibrated"] and status == "GOOD POSTURE") else (0,0,255) if (ref_values["calibrated"] and status == "BAD POSTURE") else (255,255,0)
                if status == "BAD POSTURE":
                    self.gif_holder["bad"].overlay_next_frame(frame)
                    if self.prev_status != "BAD POSTURE":
                        print("Plays Music")
                        pygame.mixer.stop()
                        # pygame.mixer.Sound(sounds["bad"]).play(loops=-1)
                    ...
                    if self.prev_status != "GOOD POSTURE":
                        pygame.mixer.stop()
                        # pygame.mixer.Sound(sounds["good"]).play(loops=-1)
                ...
                if ref_values["calibrated"]:
                    cv2.putText(frame, f"Reference Nose-Shoulder Dist: {ref_values["ref_nose_shoulder_dist"]:.3f}", (20,base_y + 60), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0,255,0), 1)
            ...
            if key == ord('q'): break
                ...
                if res.pose_landmarks:
                    ...
                    ref_values["ref_nose_z"] = nose.z
                    ref_values["ref_shoulder_z"] = (L.z + R.z) / 2
                    ref_values["ref_shoulder_y"] = (L.y + R.y) / 2
                    ref_values["ref_nose_shoulder_dist"] = abs(nose.y - shoulder_y)
                    ref_values["calibrated"] = True
...
posture_detector = PostureDetector(ref_values= ref_values, settings= settings, gif_holder= gif_holder, sounds= sounds)
settings.attach_detector(posture_detector)
cv_thread = Thread(target=posture_detector.run, daemon=True)
cv_thread.start()
